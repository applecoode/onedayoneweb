...
client = MongoClient()
mongodb = client.local.tjy_xzqh_2016
...
def catgroy_xzqh(url):#按省市县分类
    ...
    if soup_city.find(class_= 'provincetr'):
        ...
        flag = 2
    return xzqh_list,urlsub,flag


def get_url(list,flag):#正式获取函数
    ...
    if flag == 0:
        for i in list:
            re.match()
            xzqh_list_url.append(base_url+i['href'])
            xzqh_dict = {}
        ...
        for i in range(len(list)//2):
            if re.
            xzqh_list_url.append(basurl+list[i*2]['href'])
            xzqh_dict={}
...
#url4 = 'http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2016/37/3714.html'
# city_list = []
# city_urllist = []
